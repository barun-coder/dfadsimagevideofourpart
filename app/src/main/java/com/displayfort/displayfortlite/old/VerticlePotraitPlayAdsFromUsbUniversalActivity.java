 ...
 * MyApplication
 */
public class VerticlePotraitPlayAdsFromUsbUniversalActivity extends BaseSupportActivity implements SurfaceHolder.Callback {
    ...
    private void check() {
        try {
            ...
            for (File inFile : files) {
                Toast.makeText(VerticlePotraitPlayAdsFromUsbUniversalActivity.this, "" + inFile.getAbsolutePath(), Toast.LENGTH_SHORT).show();
...
//            startService(intent);
    ...
    private void startServiceVideoProcess() {
        ...
        File demoVideoPath = new File(demoVideoFolder);
        if (!demoVideoPath.exists()) {
            demoVideoPath.mkdirs();
        }
        if (completFileList != null && completFileList.length > 0) {
            for (int i = 0; i < completFileList.length; i++) {
                ...
                if (type != null && type.toLowerCase().contains("video") && !transposeFile.exists()) {// && getFileExtension(files).equalsIgnoreCase("mp4")
                    ...
                    StartCompress(files.getAbsolutePath(), transposeFile.getAbsolutePath());
                }
            }
        }
    }

    private void StartCompress(final String srcPath, final String destPat) {
//        new Mp4Composer(srcPath, destPat)
...
//                        exportMp4ToGallery(getApplicationContext(), destPat);
//                        runOnUiThread(new Runnable() {
//                            @Override
//                            public void run() {
//                                Toast.makeText(VerticlePotraitPlayAdsFromUsbUniversalActivity.this, "codec complete path =" + destPat, Toast.LENGTH_SHORT).show();
//                            }
//                        });
//
//                    }
